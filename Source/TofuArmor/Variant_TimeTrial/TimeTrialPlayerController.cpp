	...
	if (IsLocalPlayerController())
	{
		if (ShouldUseTouchControls())
		{
			...
			if (MobileControlsWidget)
			{
				...
				UE_LOG(LogTofuArmor, Error, TEXT("Could not spawn mobile controls widget."));
		...
		if (UIWidget)
		{
			...
			UE_LOG(LogTofuArmor, Error, TEXT("Could not spawn Time Trial UI widget."));

		}
		

		// spawn the UI widget and add it to the viewport
		VehicleUI = CreateWidget<UTofuArmorUI>(this, VehicleUIClass);
		...
		if (VehicleUI)
		{
			...
			UE_LOG(LogTofuArmor, Error, TEXT("Could not spawn vehicle UI widget."));
...
void ATimeTrialPlayerController::OnPossess(APawn* InPawn)
{
	...
	VehiclePawn = CastChecked<ATofuArmorPawn>(InPawn);
...
void ATimeTrialPlayerController::OnPawnDestroyed(AActor* DestroyedPawn)
{
	...
	if (ActorList.Num() > 0)
	{
		...
		if (ATofuArmorPawn* RespawnedVehicle = GetWorld()->SpawnActor<ATofuArmorPawn>(VehiclePawnClass, SpawnTransform))
		{
			// possess the vehicle
